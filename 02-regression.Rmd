...
X <- model.matrix(fm0)
f1 <- function(b) with(cars, sum(log(cosh(dist - X %*% b))))
res <- optim(coef(fm0), f1, method = "BFGS")
res$par

f2 <- function(b) with(cars, mean(abs((dist - X %*% b))))
...
param <- param %>% 
  bind_rows() %>% 
  rename(Intercept = 1) %>% 
  mutate(loss = c("Log-Cosh", "MAE"))

cars %>% 
  ggplot(aes(speed, dist)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  geom_abline(data = param, aes(intercept = Intercept, slope = speed, color = loss))
```
